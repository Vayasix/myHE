...
int setParam(int argc, char **argv)// param of host and so on.
{
	int opt;
	int nopt=0;
	int core=1;
...
//	ROT::SysInit();// see ROT.cpp
	...
	std::string contf = key_dir_path + "context";
	c.setParam(vlen, B0, L0, L1, (char *)file.c_str(), (char *)prvkf.c_str(), (char *)contf.c_str());
	c.core = core;
	...
	sentSize += sendFile(sock, (char *)contf.c_str()); //send context to server
...
//}
		...
		if(isL!=0){
			...
			c.decResult(resultf, (ff/c.L1)%c.L0, ff%c.L1);
			ff = c.L1*c.t0 + c.t1;
			
			DBG_PRT("dec result g\n");	
			c.decResult(resultg, (gg/c.L1)%c.L0, gg%c.L1);
			gg = c.L1*c.t0 + c.t1;
