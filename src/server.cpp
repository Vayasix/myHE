...
//	ROT::SysInit();
	...
	while(1){
		...
		std::string file2 = tmp_dir_path + "c2s_context";
		recvFile(sock, (char *)file.c_str()); //receive pubkey from client
		recvFile(sock, (char *)file2.c_str()); //receive context from client
		s.core = core;
		s.setParam(file,file2);//set context and pubkey and ea
		
		s.updtLUTable();
		std::cerr << "context and pub and ea end" << std::endl; 
...
//#pragma omp section
		...
		//	}
			...
			//	s.makeIsELongest(ismatch, epsilon);//FIXME later
			}
			wte = get_wall_time();
			calc_total += wte - wts;
